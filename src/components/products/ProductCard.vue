...
const props = defineProps<{ itemData: ISneakersProduct }>();
</script>
...
<template>
    ...
    >
        <img
            ...
            :src="props.itemData.isFavorite ? '/like-2.svg' : '/like-1.svg'"
        />
        <img
            class="product-card__img max-h-28"
        ...
        <div class="product-card__footer flex w-full items-center justify-between">
            <div class="product-card__price-container">
                ...
                <p class="text-sm font-bold">{{ props.itemData.price }} руб</p>
            </div>
            <img
                class="product-card__add-to-cart cursor-pointer rounded-lg hover:shadow-md"
                :src="`/${props.itemData.isAdded ? 'checked.svg' : 'plus.svg'}`"
            />
        </div>
    </div>
</template>
